...
public interface ConsultasRepository extends JpaRepository<ConsultasModel, Long>{
    ...
    @Query(value = "SELECT id_empresa FROM consulta c WHERE c.id like :idConsulta or RIGHT(c.id, :tamanhoFinal) like :ultimoDigito AND c.frequencia =:frequencia AND c.tipo_consulta = :tipoConsulta;", nativeQuery = true)
    List<Long> empresasOfDay(@Param("idConsulta") int idConsulta, @Param("tamanhoFinal") int tamanhoFinal, @Param("ultimoDigito") String ultimoDigito, @Param("frequencia") String frequencia, @Param("tipoConsulta") int tipoConsulta);
    ...
    // Consulta SQL que retorna todos os id's de empresas relacionadas às consultas dado seu tipo.
