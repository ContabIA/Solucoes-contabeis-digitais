...
public interface RespostaRepository extends JpaRepository<RespostaModel, Long>{
    ...
    @Query(value = "SELECT r.* FROM resposta r JOIN consulta c ON r.id_consulta = c.id JOIN empresa e ON c.id_empresa = e.id JOIN cliente cl ON e.id_usuario = cl.id WHERE cl.username = :cnpjUser AND r.novo = :novo AND r.status = :status", nativeQuery = true)
